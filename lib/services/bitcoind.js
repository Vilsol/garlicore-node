...
Bitcoin.DEFAULT_CONFIG_SETTINGS = {
  ...
  timestampindex: 1,
  spentindex: 1,
  zmqpubrawtx: 'tcp://127.0.0.1:42080',
  zmqpubhashblock: 'tcp://127.0.0.1:42080',
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of garlicoin-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of garlicoin-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of garlicoin-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from garlicoin. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of garlicoin-node services will start.');
    node._reindex = true;
  }
};
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.estimateSmartFee = function(blocks, callback) {
  var self = this;
 ...
 *   currentBlockTx: 0,
 *   difficulty: 53.30757498729029,
 *   errors: '',
 *   networkHashps: 5072920160.996502,
 *   pooledTx: 1,
 *   chain: 'main'
 * }
 * @param {Function} callback
